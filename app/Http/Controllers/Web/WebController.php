...
class WebController extends Controller
{
    ...
    public function home()
    {
        ...
        $home_categories->map(function ($data) use($country_code,$city_id){
            $id = '"'.$data['id'].'"';
        ...
        $top_sellers = Seller::approved()->with('shop')
            ->withCount(['orders'])->orderBy('orders_count', 'DESC')->CountryFilter()->cityFilter()->take(15)->get();
        //end
        ...
        $bestSellProduct=$bestSellProduct->select('product_id', DB::raw('COUNT(product_id) as count'))
            ->groupBy('product_id')
            ->orderBy("count", 'desc')
            ->take(4)
            ->get();
    ...
    public function all_sellers()
    {
        ...
        })->CountryFilter()->CityFilter()->paginate(24);
        return view('web-views.sellers', compact('sellers'));
    }
    ...
    public function seller_shop(Request $request, $id)
    {
        ...
        return view('web-views.shop-page', compact('products', 'shop', 'categories','seller'))
            ->with('seller_id', $id)
